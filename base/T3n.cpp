...
U8 Souliss_Logic_T31(U8 *memory_map, U8 slot, U8 *trigger)
{
	...
	// use fabs instead abs for esp8266 lib >= 2.4.0
	if((fabs(in_setpnt-actual_setpnt) > Souliss_T3n_SetPointDeadBand) && (in_setpnt > 1)) {
		actual_setpnt = in_setpnt;									// Set the new setpoint value
	}	

	// If there is a change in the new temperature
	// use fabs instead abs for esp8266 lib >= 2.4.0
	if((fabs(in_temp-actual_temp) > Souliss_T3n_DeadBand) && (in_temp > 1) && !isnan(in_temp)) {	// Sometimes during changes a "0" temperature is reported. In this case we don't want this to cause things to turn on and off. So we make sure the measured temperature is > 0
